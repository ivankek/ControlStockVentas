...
- Despachos: consulta temporal por fecha argentina mediante POST /api/sync con {date: "YYYY-MM-DD"}. Consulta ventas creadas desde 90 días antes hasta el día seleccionado; filtra por fecha del estado shipped registrado, independientemente del estado actual. No guarda pedidos, productos ni resultados en Supabase. Los resultados se descartan al salir de la pantalla. Los envíos sin fecha verificable se muestran aparte. Más de 10.000 ventas requiere particionar la consulta. No se promete historial completo. La renovación de tokens OAuth sigue persistiendo credenciales cifradas. Los datos guardados anteriormente no se eliminan.
