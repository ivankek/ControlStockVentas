...
export default function Users() {
  ...
  return <div className="inventory-view"><p className="table-note">Creá usuarios desde Supabase Authentication. Sus perfiles empiezan como USER. ADMIN se asigna por SQL seguro, nunca desde este formulario.</p><button disabled={busy} onClick={() => void run()}>Actualizar usuarios</button>
    ...
    <section className="panel stock-card"><h2>Asignar catálogo anterior</h2><p>Elegí el proveedor real de cada catálogo. Conserva productos, costos históricos, asociaciones y unidades por venta. También habilita la relación del propietario anterior con el proveedor elegido. Esta asignación se realiza una sola vez; el original se conserva.</p>
      {data.legacy.map((l) => <div className="stock-association" key={l.owner_id}><strong>{name(l.owner_id)} · {l.count} productos</strong>{l.assigned_supplier_id ? <p>Asignado a {name(l.assigned_supplier_id)}</p> : <form className="inventory-form" onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); void run({ type: "assignLegacy", ownerId: l.owner_id, supplierId: f.get("supplier") }); }}><label>Proveedor propietario<select required name="supplier"><option value="">Elegir proveedor</option>{suppliers.map((p) => <option value={p.id} key={p.id}>{p.display_name}</option>)}</select></label><label className="inventory-confirm"><input required type="checkbox" /> Confirmo que este catálogo pertenece al proveedor seleccionado</label><button disabled={busy}>Asignar catálogo</button></form>}</div>)}
      {!data.legacy.length && <p>No hay catálogos anteriores para asignar.</p>}
    </section>
  </div>;
}
