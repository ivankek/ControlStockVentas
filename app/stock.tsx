...
export default function Stock() {
  ...
  async function save(v: StockVariant, physical: number) {
    ...
    try {
      await command({ type: "adjust", variantId: v.id, delta: physical - v.physical_stock, reserved: v.reserved_stock, safety: v.safety_stock, expectedVersion: v.version, movementType: "CORRECTION", note: "Actualización manual del stock físico", requestId: crypto.randomUUID() });
  ...
  return <div className="inventory-view">
    <p className="notice">Modificá directamente el stock físico. El vendible se calcula descontando reservas y stock de seguridad.</p>
    ...
    <section className="panel stock-grid-panel">
      ...
      <div className="table-wrap"><table className="stock-grid"><thead><tr><th>Producto</th><th>SKU</th><th>Físico</th><th>Reservado</th><th>Seguridad</th><th>Vendible</th><th>Acciones</th></tr></thead><tbody>
...
function StockRow({ variant: v, supplier, mappings, expanded, busy, canEdit, onToggle, onSave }: { variant: StockVariant; supplier: string; mappings: StockMapping[]; expanded: boolean; busy: boolean; canEdit: boolean; onToggle: () => void; onSave: (v: StockVariant, physical: number) => Promise<void> }) {
  ...
  return <>
    <tr>
      ...
      <td>{canEdit ? <input className="stock-number" aria-label={`Stock físico de ${v.product_name}`} type="number" min={0} max={1000000000} step={1} value={physical} onChange={(e) => setPhysical(Math.max(0, Number(e.target.value)))} /> : v.physical_stock}</td>
      <td>{v.reserved_stock}</td><td>{v.safety_stock}</td><td><strong>{sellableStock(v.physical_stock, v.reserved_stock, v.safety_stock)}</strong></td>
      <td><div className="stock-actions">{canEdit && <button className="primary stock-save" disabled={busy || !changed} onClick={() => void onSave(v, physical)}>Guardar</button>}<button onClick={onToggle}>{expanded ? "Ocultar" : "Publicaciones"} ({mappings.length})</button></div></td>
    </tr>
    {expanded && <tr className="stock-expanded"><td colSpan={7}><strong>Publicaciones asociadas</strong>{mappings.length ? mappings.map((m) => <div className="stock-association" key={`${m.account_id}:${m.item_id}:${m.variation_id}`}><strong>{m.title}</strong><span>{m.item_id}{m.variation_id !== "0" ? ` · variante ${m.variation_id}` : ""}</span><small>{m.nickname ?? "Cuenta ML"} · Stock ML: {m.ml_quantity ?? "No disponible"} · Deseado: {desiredListingQuantity({ mode: m.mode, sellableStock: sellableStock(v.physical_stock, v.reserved_stock, v.safety_stock), fixedQuantity: m.fixed_quantity })}</small></div>) : <p className="table-note">No hay publicaciones asociadas.</p>}</td></tr>}
  </>;
}
