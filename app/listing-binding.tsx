...
export default function ListingBinding({ keys, policyOnly = false }: { keys: string[]; policyOnly?: boolean }) {
  ...
  if (policyOnly) return v ? <p className="table-note">Stock a enviar a Mercado Libre: {v.stock}. Se actualiza al guardar una asociación o cambiar el stock del proveedor.</p> : null;
  return <form className="supplier-binding" key={JSON.stringify(mappings)} onSubmit={async (e) => {
    ...
    try {
      if (policyOnly) {
        ...
        await command({ type: "mapping", accountId: account, keys, variantId: String(f.get("variant")) || null, units: Number(f.get("units")), mode: "REAL", fixed: null, preservePolicy: false });
      }
      setMessage("Asociación guardada. Revisá arriba el estado de sincronización del stock.");
    ...
    {!policyOnly ? <><label>Producto / variante del proveedor<select name="variant" defaultValue={mixed ? "" : current?.variant_id ?? ""} disabled={busy}><option value="">Sin asociación</option>{data.variants.map((v) => <option key={v.id} value={v.id}>{data.people.find((p) => p.id === v.supplier_id)?.display_name} · {v.product_name}{v.name === "Única" ? "" : ` · ${v.name}`}</option>)}</select></label>
      ...
      {v && <small>Deseado guardado: {v.stock} · Se sincroniza con Mercado Libre</small>}
    </>}
