...
export default function Profits({ token }: { token?: string }) {
  ...
  async function consult() {
    ...
    try {
      ...
      for (let from = range.from; from <= range.to; from = shift(from, 7)) {
        const to = shift(from, 6) > range.to ? range.to : shift(from, 6);
        let offset: number | null = 0;
        let expectedTotal: number | undefined;
        while (offset !== null) {
          setProgress(`${from} al ${to} · ${sales.size} ventas consultadas`);
          const page = await json("/api/profit", { from, to, offset });
          if (expectedTotal !== undefined && expectedTotal !== page.total) throw Error("Las ventas cambiaron durante la consulta. Volvé a consultar para obtener un resultado consistente.");
          expectedTotal = page.total;
          for (const sale of page.sales as Sale[]) {
      ...
      if (!request.signal.aborted) setResult({ sales: [...sales.values()], state, ...range, time: new Date().toLocaleString("es-AR") });
  ...
  return <>
    ...
    {busy && <p role="status">{progress || "Preparando consulta…"} <button onClick={() => { controller.current?.abort(); setBusy(false); setProgress(""); }}>Cancelar</button></p>}
    ...
    {totals && result && <>
      ...
      <section className="panel pending"><div className="panel-title"><h2>Detalle por venta</h2></div><div className="table-wrap"><table><thead><tr><th>VENTA / FECHA</th><th>BRUTO</th>{net && <><th>RECIBIDO</th><th>PROVEEDOR</th><th>LOGÍSTICA PROPIA</th><th>NETO ANTES DE GASTOS MENSUALES</th></>}</tr></thead><tbody>
        {rows.map((row) => <tr key={row.sale.id}><td><strong>{row.sale.id}</strong><p>{row.date} · {row.sale.mode}</p><small>{row.sale.province} · {row.sale.city}</small>{net && <><p>{row.issues.join(" · ")}</p><OrderNoteEditor financial order={row.sale} note={business.notes[row.sale.id]} token={token} onSaved={() => void refreshCosts()} /></>}</td><td>{row.gross === undefined ? "A revisar" : money(row.gross)}</td>{net && <><td>{row.received === undefined ? "Pendiente" : money(row.received)}{row.manualNet && <small>Manual</small>}</td><td>{row.supplier === undefined ? "Pendiente" : money(row.supplier)}</td><td>{row.shipping === undefined ? "Pendiente" : money(row.shipping)}</td><td>{row.net === undefined ? "Pendiente" : money(row.net)}</td></>}</tr>)}
      </tbody></table></div>{!rows.length && <p className="empty">No se encontraron ventas en el período consultado.</p>}</section>
    </>}
  </>;
}
