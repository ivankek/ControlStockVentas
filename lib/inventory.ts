...
export const inventoryCommand = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("product"), supplierId: uuid, productId: uuid.optional(), variantId: uuid.optional(), name: z.string().trim().min(1).max(200), sku: z.string().trim().min(1, "El SKU es obligatorio.").max(100), date: z.iso.date(), cents: z.number().int().min(0).max(100000000000), stock: quantity, expectedVersion: quantity.optional() }).refine((v) => !!v.productId === !!v.variantId && (!v.variantId || v.expectedVersion !== undefined), "Actualizá el producto antes de guardar."),
  z.object({ type: z.literal("adjust"), variantId: uuid, delta: z.number().int().min(-1000000000).max(1000000000), expectedVersion: quantity, movementType: z.enum(["MANUAL_ADJUSTMENT", "RESTOCK", "CORRECTION"]), note: z.string().trim().min(1).max(500), requestId: uuid }),
